...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-slate-900">Nuevo logro</h2>
        <p class="mt-1 text-sm text-slate-500">Crea un nuevo logro para la plataforma.</p>
    </div>

    <div class="bg-white shadow-sm ring-1 ring-slate-900/5 sm:rounded-xl">
        <form action="{{ route('admin.achievements.store') }}" method="POST" class="px-4 py-6 sm:p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="mt-2">
                        <input type="text" name="icon" id="icon"
                            value="{{ old('icon') }}" required maxlength="10"
                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm">
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        {{-- Select con los 4 tipos que el AchievementChecker sabe manejar --}}
                        <select name="condition_type" id="condition_type" required
                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm">
                            <option value="">— Selecciona un tipo —</option>
                            <option value="total_logs" @selected(old('condition_type') === 'total_logs')>Total de sesiones (total_logs)</option>
                            <option value="streak_days" @selected(old('condition_type') === 'streak_days')>Días de racha (streak_days)</option>
                            <option value="total_xp" @selected(old('condition_type') === 'total_xp')>XP acumulado (total_xp)</option>
                            <option value="completed_books" @selected(old('condition_type') === 'completed_books')>Libros completados (completed_books)</option>
                        </select>
                ...
                <div class="sm:col-span-3">
                    <label for="condition_value" class="block text-sm font-medium text-slate-900">
                        Valor de la condición
                    </label>
                    <div class="mt-2">
                        <input type="number" name="condition_value" id="condition_value"
                            value="{{ old('condition_value') }}" min="1" required
                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm">
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        <input type="number" name="xp_reward" id="xp_reward"
                            value="{{ old('xp_reward', 0) }}" min="0" required
                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm">
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-2">
                        <textarea name="description" id="description" rows="3" required
                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm">{{ old('description') }}</textarea>
