...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white shadow-sm ring-1 ring-slate-900/5 sm:rounded-xl">
        <form action="{{ route('books.reading-logs.store', $book) }}" method="POST" class="px-4 py-6 sm:p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-2">
                        ...
                        <p class="mt-2 text-xs text-slate-500">Se registra automáticamente con la fecha de hoy.</p>
                    </div>
                </div>
                ...
                <div class="sm:col-span-3">
                    <label for="pages_read" class="block text-sm font-medium leading-6 text-slate-900">
                        Páginas leídas <span class="text-slate-400">({{ $pagesRemaining }} restantes)</span>
                    </label>
                    <div class="mt-2">
                        <input type="number" name="pages_read" id="pages_read"
                            value="{{ old('pages_read') }}" min="1" max="{{ $pagesRemaining }}" required
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-2">
                        <textarea name="comments" id="comments" rows="4"
                            class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">{{ old('comments') }}</textarea>
